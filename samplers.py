# Adapted from: https://pytorchnlp.readthedocs.io/en/latest/_modules/torchnlp/samplers/bucket_batch_sampler.html
...
def sentence_length(x, sampler):
    return sum(x > 0 for x in sampler.data_source[x]['input_ids'])
...
class SortedSampler(Sampler):
    ...
    def __init__(self, data, sampler, sort_key=sentence_length):
        super().__init__(data)
        self.data = data
        self.sort_key = sort_key
        zip_ = [(row, self.sort_key(row, sampler)) for i, row in enumerate(self.data)]
...
class BucketBatchSampler(BatchSampler):
    ...
    def __init__(self,
                 ...
                 batch_size,
                 drop_last,
                 sort_key=sentence_length,
                 bucket_size_multiplier=100):
        super().__init__(sampler, batch_size, drop_last)
        self.sort_key = sort_key

        # Determine the bucket size: the dataset is divided in buckets of that
        # size using BatchSampler.
        ...
        # Used to access the sentence length of the samples
        self.sampler_ = sampler

    def __iter__(self):
        for bucket in self.bucket_sampler:
            # Order indices by sentence length
            sorted_sampler = SortedSampler(bucket, self.sampler_, self.sort_key)
            # Randomly return segments of indices (each segment including
            # sentences of similar length)
            for batch in list(
                BatchSampler(sorted_sampler, self.batch_size, self.drop_last)
            ):
                yield batch
